<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Purchase.aspx.cs" Inherits="Components_Purchase" %>
...
<body>
    ...
    <script>
        ...
        $('#txt_search').autocomplete({
            source: function (request, response) {
                ...
                if (Search_Value.length >= 2) {
                    ...
                    $.ajax({
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        url: "Purchase.aspx/fnGetAllProducts",
            ...
            create: function (event, ui) {
                $(this).data('ui-autocomplete')._renderItem = function (ul, item) {
                    ...
                    //return $('<li>').append('<div class="row"><div class="col-2" ><img class="w-100" src="' + 'https://mycornershop.in/Menu_Pics/' + item.IMAGE_URL + '" alt="img">' +
                    //    '</div><div class="col-10"><div class="row"><div class="col-12"><label>' + item.Item_Name + ' - Rs.' + item.Price + '</label></div></div>' +
                    ...
                    //    '<button class="btn btnstyle itemListLi" type="button" item_name="' + item.Item_Name + '" mid="' + item.MID + '">add</button></div></div></div></div></div>').appendTo('#searchResult');
        ...
        $('#callPersonalDetailSection').click(function (e) {
            ...
            if ($('#item_count').text() != '0') {
                ...
                var Data = '<div class="row">' +
                    ...
                    '<div class="col-sm-6">' +
                    '<div class="card" style="padding: 10px; margin-top: 20%;"><form>' +
                    ...
                    '<span class="highlight"></span><label>Name</label></div><div class="floating-label"><input class="floating-input"  id="Mob"  type="number" placeholder=" "><div id="results"></div>' +
                    '<span class="highlight"></span><label>Mobile</label></div><div class="floating-label"><textarea style="height: 100px;" class="floating-input"  id="Address"></textarea>' +
                    '<span class="highlight"></span><label>Address</label></div><div></div></div></div></div><div class="row"><div class="col-5 d-none"><div class="form-group"><label for="pwd"  class="fu_input">Amount</label>' +
                    '<input type="number" class="form-control" value="' + $.trim($('#item_total_amount_').text()) + '" id="amts"></div></div>' +
                    '<div class="container"><div style="margin-left:5px;margin-bottom:20px;" class="row"><div class="col-sm-12 form-check "><input type="checkbox" class="form-check-input" id="Delivered" name="option1">' +
                    '<label class="form-check-label" for="Delivered">Delivered</label></div></div><div class="row mb-2"> <div class="col-7"><div class="form-check"><input type = "checkbox" checked="checked" class="form-check-input" id = "exampleCheck1">' +
                    '<label class="form-check-label" for="exampleCheck1">Paid for full</label></div></div><div class="col-5">' +
                    '<input type="number" readonly="readonly" class="w-100 editedamountcss" value="' + $.trim($('#item_total_amount_').text()) + '" id="amt"/></div></div></div>' +
                    '</div ></div>' +
                    '<button type="button" class="btn btnstyle w-100" id="submitOrder">Submit</button></form></div></div>' +
                    '<div class="col-sm-3"></div></div>';
                $('#Personal_Section').empty();
                ...
                $('#submitOrder').click(function (f) {
                    ...
                    var deliveryflag = $("#Delivered").prop('checked') == true ? "Y" : "N";
                    if ($('#item_count').text() != '0') {
                        $('#submitOrder').attr('disabled', 'true');
                        CreateOrder(Mobile, Name, Amount, 'ADD', address, deliveryflag, AddressId);
                    }
                    else {
                        showpopup('Create order first');
                    }
                ...
                $("#Mob").keyup(function () {
                    ...
                    if (mobile.length == 5) {                       
                        resultsDiv.empty();       
                        resultsDiv.show();
                        Getaddressbymobilenumber(mobile);
                    } else {
                        resultsDiv.empty();
                        resultsDiv.hide();  
                    }
        ...
        function CreateOrder(Mobile, Name, Amount, Flag, address, deliveryflag, AddressId) {
            if (Offer_Data == '') {
                inittialiseOrderData();
            }
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "Purchase.aspx/CreateOrder",
                data: '{Mobile: "' + Mobile + '",Name:"' + Name + '", OfferData:' + Offer_Data
                    + ', Flag:"' + Flag + '", Paid_Amount:"' + Amount + '", Address:"' + address + '", deliveryflag:"' + deliveryflag + '", AddressId:"' + AddressId + '"}',
                dataType: "json",
                success: function (data) {
        ...
        function getSaleHistory() {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "Purchase.aspx/fngetSaleHistory",
                data: '{}',
                dataType: "json",
                beforeSend: function () {
                    $("#preload").show();
                    ...
                    if (data.d != '') {
                        ...
                        $('.generate_Invoice').click(function (e) {
                            ...
                            $.ajax({
                                type: "POST",
                                contentType: "application/json; charset=utf-8",
                                url: "Purchase.aspx/fnGenerateInvoice",
        ...
        function Getaddressbymobilenumber(mobile) {

            var query = mobile;
            $.ajax({
                type: 'POST',
                url: 'Purchase.aspx/GetSuggestions',
                data: JSON.stringify({ Mobile: query }),
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (data) {
                    var resultsDiv = $('#results');
                    ...
                    if (data.d.length > 0) {
                        data.d.forEach(function (item) {
                            resultsDiv.append('<li onclick="fnaddressselection($(this))" AddressId=' + item.Address_ID + '><div class="nameuser" style="font-weight:bold;">' + item.Name + '</div><div class="addressuser" style="font-size: 11px;color: black;font-weight:bold;">' + item.Comment + '</div></li>');
                        });
                    } else {
                        resultsDiv.append('<div>No results found. <button onclick="fnclosesuggestionbox($(this))" type="button" class="btn btnstyle w-100">Close</button></div>');
                    }
                },
                error: function () {
        ...
        function fnaddressselection(current) {
            var resultsDiv = $('#results');
            $("#name").val(current.find('.nameuser').text());
            $("#Address").val(current.find('.addressuser').text());
            var addressid = current.attr("AddressId");
            $("#Address").attr("AddressId", addressid);
            resultsDiv.empty();
            resultsDiv.hide();
        }

        function fnclosesuggestionbox(current) {
            var resultsDiv = $('#results');
            resultsDiv.empty();
            resultsDiv.hide();
        }
