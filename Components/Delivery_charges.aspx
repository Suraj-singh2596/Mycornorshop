<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Delivery_charges.aspx.cs" Inherits="Components_Delivery_charges" %>
...
<body>
    ...
    <nav class="navbar fixed-top navbar-light bg-light" id="headertextappend">
        <a href="javascript:void(0);" id="StoreName" runat="server" class="navbar-brand" style="margin-left: 40px; margin-top: -15px;">Delivery Charge
            ...
            <div class="row">
                <div class="col-md-10 m-x-auto pull-xs-none" style="margin: auto;">
                    <div class="jumbotron">
                        <div class="row">
                            <div class="col-8">
                                <h2 class="h2-responsive" style="font-size: 20px;color: #17c3d1;"><strong>Delivery Charges</strong></h2>
                            </div>
   ...
   <%--  <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.3.0/js/bootstrap-datepicker.js"></script>
    ...
    <script>
        ...
        $("#adddeliverySlotbtn").click(function (e) {
            ...
            GetDeliveryChargesDetails("M");
        })

          function GetDeliveryChargesDetails(flag) {
            $.ajax({
                type: "POST",
                async: true,
                url: "Delivery_charges.aspx/GetDeliveryDetails",
                data: '{"flag":"' + flag + '"}',
                ...
                success: function (data) {
                    if (data.d != "") {
                        if (flag == "L") {
                            ...
                            $("#basicModal").empty();
                            $("#basicModal").append(data.d);
                            $("#basicModal").modal();

                            $("#finalsubmit").click(function (e) {
                                e.stopImmediatePropagation();
                                var deliveryA = $("#DelievryYes :selected").val();
                                var deliveryradius = $("#Radius").val();
                                var amount = $("#Minimumamt").val(); 
                                var charge = $("#Charge").val(); 
                                var Delievryalways = $("#Delievryalways :selected").val();
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "Delivery_charges.aspx/updatedeliverycharge",
                                    data: '{"deliveryA":"' + deliveryA + '","deliveryradius":"' + deliveryradius + '","amount":"'
                                        + amount + '","charge":"' + charge + '","Delievryalways":"' + Delievryalways + '"}',
                                    contentType: "application/json; charset=utf-8",
                                    dataType: "json",
                                    success: function (data) {
                                        if (data.d != "") {
                                            showpopup("Record updated successfuly")
