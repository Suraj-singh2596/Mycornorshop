<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Delivery.aspx.cs" Inherits="Components_Delivery" %>
    ...
    <form id="form1" runat="server">
        <div>
            ...
            <nav class="navbar fixed-top navbar-light bg-light">
                <a href="javascript:void(0);" id="StoreName" runat="server" class="navbar-brand" style="margin-left: 40px;"></a>
                <a id="Notif_seenBtn" style="color: white; position: fixed; right: 0; top: 20px;">
            ...
            <div class="container" style="margin-top: 18px;">
                ...
                <div class="row mb-2 pt-4">
                    ...
                    <div class="col-sm-6">
                        <div class="row" style="position: fixed; width: 100%; margin-top: -10px; z-index: 9999;">
                            <div class="col-12">
        ...
        <%--<div class="modal-dialog" role="document">
            <div class="modal-content" style="margin-top: 40%;">
                ...
                <div class="modal-body">
                   <label>Order  #this : <span>8/10 item(s) is(are) available.</span></label>
                    <span></span>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn  btnstyle">Notify Customer</button>
                </div>
            </div>
        </div>--%>
    </div>
    ...
    <script>
        $('#view_more').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "Delivery.aspx/getCustomerOrdersbydatesNextRecords",
        ...
        function switchToggle() {
            $('.switcher input').change(function (e) {
                ...
                if ($(this).prop('checked')) {
                    fnParticularItemCheckedUnchecked('', $(this).attr('lineid'), 'Y', '')
                }
                else {
                    fnParticularItemCheckedUnchecked('', $(this).attr('lineid'), 'N', '')
                }
            })
        }
        function All_Actions() {
            ...
            $(".show__itemsmore").click(function () {
                ...
                if ($.trim(current.parent().parent().parent().find(".items__append").html()).length == 0) {
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "Delivery.aspx/getItemsByHeader",
            ...
            $(".recivedamt").click(function (e) {
                ...
                $(".proceedpaybtn").click(function () {
                    ...
                    getDataByHeaderID($(this).attr("headerid"), current, 'N');
                    $("#paymodal").modal('hide');
                })
                ...
                function payamountfunction(recievedamt, MOP, Header_ID) {
                    if (recievedamt == '0') {
                        ...
                        $.ajax({
                            type: "POST",
                            async: true,
                            url: "Delivery.aspx/AmountReceived",
            ...
            $('.notifybtnshow').click(function (e) {
                ...
                var bodytext = '<label class="textboxdiv">You will no longer can make changes if you notify customer</span></label>';
                var notifmodalD = '<div class="modal-dialog" role="document">' +
                    '<div class="modal-content" style="margin-top: 40%;" >' +
                    '<div class="modal-header border-0">' +
                    '<h5 class="modal-title" id="exampleModalLabel">Freezing Order Alert</h5>' +
                    '<button type="button" class="close" data-dismiss="modal" aria-label="Close">' +
                    '<span aria-hidden="true">&times;</span>' +
                    '</button>' +
                    '</div>' +
                    ...
                    '<div class="modal-footer border-0">' +
                    '<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>' +
                    '<a  href="javascript:void(0);" id="notifyCustomerBtn" class="btn btnstyle notifycustbtn"   headerid="' + Header_ID + '">Notify Customer </a>' +
                    '</div></div ></div >';
                $("#notifymodal").empty();
                $("#notifymodal").append(notifmodalD);
                $("#notifymodal").modal();

                $('#notifyCustomerBtn').click(function () {

                    showpopup('Notified to customer successfully');
                    getDataByHeaderID(Header_ID, current, 'Y');
                    $("#notifymodal").modal('hide');
                })

            })

            function notified(Header_ID) {

                showpopup('Notified to customer successfully11');
                $.ajax({
                    type: "POST",
                    url: "Delivery.aspx/NotifyCustomer123",
                    ...
                    success: function (data) {
                        showpopup('Notified to customer successfully1');
            ...
            function getDataByHeaderID(Header_ID, current, Flag) {

                $.ajax({
                    type: "POST",
                    url: "Delivery.aspx/getCustomerOrdersbydatesRefresh",
                    data: '{Header_ID:"' + Header_ID + '",Flag:"' + Flag + '"}',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    beforeSend: function () {
                        $("#preload").show();
        ...
        function getData(startdate, enddate) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "Delivery.aspx/getCustomerOrdersbydates",
        ...
        function fnParticularItemCheckedUnchecked(Header_ID, Line_ID, Flag, Order_No) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "Delivery.aspx/MarkDelivered",
        ...
        function todaysCollection() {
            $.ajax({
                type: "POST",
                async: true,
                url: "Delivery.aspx/todaysCollection",
                data: '{}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
        ...
        $("#Notif_seenBtn").click(function (e) {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "Delivery.aspx/Fn_updateSeenFlag",
                data: '{}',
                beforeSend: function () {
                    $("#preload").show();
                },
