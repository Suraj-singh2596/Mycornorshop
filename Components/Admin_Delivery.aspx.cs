...
public partial class Components_Admin_Delivery : System.Web.UI.Page
{
    ...
    public void getAdminDetails()
    {
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            StoreName.InnerHtml = "Pending Deliveries <p style=\"font-size:12px;color:#e6e6e6;text-align: left;margin-bottom: 0px;font-weight: 400;visibility: hidden;\">" +
    ...
    public static string getCustomerOrdersbydates(string startdate, string enddate)
    {
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow DR in ds.Tables[0].Rows)
            {
                ...
                if (DR["CourierThrough"].ToString() != "")
                {
                    courier = "<div class='col-sm-12'><div style='color: white;padding: 2px;background: #27cacc;border-radius: 5px;text-align: center;'>Courier Through:<strong>" + DR["CourierThrough"].ToString() + "</strong></div></div>";
                }
                ...
                if (DR["CONFIRMED_ORDER"].ToString() == "Y")
                {
                    ...
                    Orders = Orders + "<div class=\"card container p-1 mt-2 \"><div class=\"row refresh\" orderid='" + DR["ORDER_NO"].ToString() + "' header_id='" + DR["HEADER_ID"].ToString() + "'><div class=\"col-md-12\"><div class=\"container\">" +
                                   ...
                                   "</div></div>" + courier + "</div></div><hr class=\"mt-1\"/><div class=\"itembox m-2 pt-2 pb-2 \"><div class=\"row \">" +
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow DR in ds.Tables[0].Rows)
                {
                        ...
                        Orders = Orders + "<div class=\"card container p-1 mt-2 \"><div class=\"row refresh\" orderid='" + DR["ORDER_NO"].ToString() + "' header_id='" + DR["HEADER_ID"].ToString() + "'><div class=\"col-md-12\"><div class=\"container\">" +
                                    ...
                                    "</div></div>" +
                                       ...
                                       "</div><div style=\"border-top: 1px solid lightgray; margin-top: 5px; padding-top: 10px; color: #2ab5b7; padding: 5px;\"><div class=\"amountsection\">" +
                                          ...
                                          "<div class=\"col-6 " + Create_invoice + "\"><a style=\"font-size:13px;\" class=\"btn btnstyle invoice w-100 text-white mt-2 p-0\" flag=\"N\" header_id='" + DR["HEADER_ID"].ToString() + "' invoice='" + DR["INVOICE_ID"].ToString() + "'>Generate Invoice</a></div>" +
                                           "<div class=\"col-6 " + view_invoice + "\"><a style=\"font-size:13px;\" class=\"btn btnstyle invoice w-100 text-white mt-2 p-0\" flag=\"Y\" header_id='" + DR["HEADER_ID"].ToString() + "' invoice='" + DR["INVOICE_ID"].ToString() + "'>View Invoice</a></div>" +
                                           "<div class=\"col-6\"><a style=\"font-size:13px;background: linear-gradient(107deg,rgb(163 163 163)0%,rgb(131 131 131)100%)!important;\" class=\"btn btnstyle delievrymark w-100 text-white mt-2 p-0\" flag=\"Y\" headerid='" +
                    ...
                    else if (DR["CONFIRMED_ORDER"].ToString() == "N")
                    {
                        ...
                        Orders = Orders + "<div class=\"card container p-1 mt-2 \" style=\"border: 2px solid grey;\" ><div class=\"row\" orderid='" + DR["ORDER_NO"].ToString() + "' header_id='" + DR["HEADER_ID"].ToString() + "'><div class=\"col-md-12\"><div class=\"container\">" +
                                       "<div class=\"row\" style=\"border-radius: 5px;color: white;\"><div class=\"col-8 col-md-8  p-0\"><div class=\"retailrdetails\">" +
                                       "<div class=\"rtname\">" + custname + " - <a style=\"color:white\" href=\"tel:" + DR["C_NUMBER"].ToString() + "\"> " + DR["C_NUMBER"].ToString() + "</a> </div><div>" + MobileNum + "</div><div style=\"font-size: 11px;color: #989898; font-weight: 500;\">" + DR["CUSTOMER_ADDRESS"].ToString() + "</div>" +
                                       "</div></div><div class=\"col-4 col-md-4  p-0\"><div class=\"orderdate__number text-center\">" +
                                       "<div class=\"order__date\">" + Convert.ToString(Convert.ToDateTime(DR["ORDER_DATE"]).ToString("dd-MMM-yyy")) + " </div><div class=\"order__number d-none\">Order No: # " + DR["ORDER_NO"].ToString() + " </div><select class=\"assign_to_select d-none \"><option>" + DR["DELIVER_BOY_NAME"].ToString() + "</option></select>" +
                                       "</div></div></div></div><hr class=\"mt-1\"/><div class=\"itembox m-2 pt-2 pb-2 \"><div class=\"row \">" +
                                       "<div class=\"col-5 col-md-4 \"><div class=\"itmdetails\">Total items &nbsp; <strong>" + DR["QTY"].ToString() + "</strong>  </div>" +
    ...
    [WebMethod]
    public static string MarkDeliveredforbtn(string Header_ID, string Line_ID,
        string Flag, string Order_No, string deliverytype, string Comment, string ImgSource)
    {
        string AdminUserId = HttpContext.Current.Request.Cookies["admin_user_id"].Value.ToString();
        ...
        cr.Line_ID = Line_ID;
        cr.Flag = Flag;
        cr.PictureUrl = savedocumentinfolder(ImgSource, Header_ID);
        cr.DispatchComments = Comment;
        cr.Type = 82;
        ds = cr.fnUpdateFlagDelivery();
        string SMS = "";
        string Notif = "";
        ...
        if (ds != null)
        {
            ...
            {
                items = items + DR["MNAME"].ToString();
            }

            if (deliverytype == "person")
            {
                SMS = "Your order-" + Order_No + " with following items " + items + " is on the way and will be delivered by PERSON with in 2-3 working days. \n This is a system generated message do not reply. Thanks \n Team ecoDENT.";
                Notif = "Your order-" + Order_No + " with following items " + items + " is on the way and will be delivered by PERSON with in 2-3 working days.";
            }
            else if (deliverytype == "courier")
            {
                SMS = "Your order-" + Order_No + " with following items " + items + " is on the way and will be delivered by COURIER with in 2-3 working days. \n This is a system generated message do not reply. Thanks \n Team ecoDENT.";
                Notif = "Your order-" + Order_No + " with following items " + items + " is on the way and will be delivered by PERSON with in 2-3 working days.";
            }
            ...
            if (ds.Tables[3].Rows.Count > 0)
            {
                if (Rid == "48") //48
                {
                    string Imageurl = getImageByHeaderId(Header_ID, Rid, AdminUserId);
                    //if (Imageurl != "")
                    //{
                    //    //string Message = "Your order at Ecodent has been dispatched. Click below to view tracking details " + Imageurl + " DoctorPlus";
                    string Message = "Your order at Ecodent has been dispatched. Click below to view tracking details " + Imageurl + " -Powered by DoctorPlus";

                    WebClient client = new WebClient();

                    string baseurl = "https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey=L6i6XjhWFUyVjT5e2CrIyg&senderid=DRPLUS&channel=2&DCS=0&flashsms=0&number=" + ds.Tables[3].Rows[0]["C_NUMBER"].ToString()
                        + "&text=" + Message.Replace("&", "and") + "&route=31&dlttemplateid=1307165036966007239";
                    Stream data = client.OpenRead(baseurl);
                    StreamReader reader = new StreamReader(data);
                    data.Close();
                    reader.Close();

                    insertNotification(AdminUserId, ds.Tables[0].Rows[0]["USER_ID"].ToString(),
                        "Dispatch SMS with image url", SMS, "Customer");
                    // }
                }
                cl_SMS.WhatsApp_Dyn_sms(ds.Tables[3].Rows[0]["C_NUMBER"].ToString(), SMS, HttpContext.Current.Request.Cookies["rid"].Value.ToString());

                Send_Notification.SendNotificationFromFirebaseecodent(Rid
               , ds.Tables[3].Rows[0]["DEVICE_ID"].ToString(), "https://mycornershop.in/Components/scheduler.aspx", "Delivery Alert", Notif, 1);

                insertNotification(AdminUserId,
                ds.Tables[0].Rows[0]["USER_ID"].ToString(), "Delivery SMS", SMS, "Customer");
    ...
    public static string getImageByHeaderId(string HeaderId, string RID, string User_ID)
    {
        string Image = "";
        DataSet ds = new DataSet();
        cl_resturant cr = new cl_resturant();
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            Image = HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority) + ConfigurationManager.AppSettings["DispatchOrderPicture"].ToString() + "/" + ds.Tables[0].Rows[0]["PictureUrl"].ToString();
        }
        return Image;
    }

    public static string savedocumentinfolder(string ImgSource, string empnum)
    {
        ...
        if (ImgSource != "")
        {
            ...
            uploadfile = GetMENUImageName(empnum);
