<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PendingDelivery.aspx.cs" Inherits="Components_PendingDelivery" %>
...
<body id="pageone">
        ...
        <div>
            ...
            <%--  <nav class="navbar fixed-top navbar-light bg-light text-center">
                <div style="width: 100%; display: flex">
                    <div style="width: 100%;">
                        <div id="restaurantnameandlogo" runat="server"><a class="navbar-brand" href="#">Gupta General Store</a> <span class="userpic"><span></span>&nbsp;&nbsp;<i class="fa fa-user-circle-o "></i></span></div>
                    </div>
                 </div>
            ...
            <div class="searchbox_box">
                <div class="row">
                    ...
                    <div class="col-sm-8">
                        <div class="container">
                            <div class="serdiv" style="margin-top: 10px; display: flex; background: #fff; font-size: 13px; cursor: pointer; width: 100%; border-radius: 5px;">
                                <div style="width: 30%;">
                                    <select id="SelectBy" style="border: none; padding: 2px; height: 26px;" class="form-control">
                                        <option value="mob">Mobile</option>
                                        <option value="header">Order No</option>
                                    </select>
                                </div>
                                <div id="" class="" style="width: 50%">
                                    <input style="border: none; padding: 2px;"
                                        class="form-control" id="search" placeholder="Search..." />
                                </div>
                                <div style="width: 20%;">
                                    <a onclick="getDataByClick()" style="border: none; padding: 2px; background: silver; color: white; width: 100%;"
    ...
    <div class="modal fade" id="DeliveryModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="padding-top: 100px;" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Delivery Type</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-6 p-2">
                            <div class="Deliverytype" typeofdel="person">
                                Dispatch in PERSON
                            </div>
                        </div>
                        <div class="col-sm-6 p-2">
                            <div class="Deliverytype deliveryactive" typeofdel="courier">
                                Dispatch through COURIER
                            </div>
                        </div>
                        <div class="col-sm-12 p-2">
                            <label>Upload Picture</label>
                            <input type="file" id="DispatchPicture" capture="camera" class="form-control" accept="image/*" />
                            <div style="padding: 10px;">
                                <img src="" style="width: 100px; display: none; height: 100px;" id="imagePreview" alt="" />
                            </div>
                        </div>
                        <div class="col-sm-12 p-2">
                            <label>Comments</label>
                            <textarea id="DispatchComments" class="form-control" placeholder="Comment here..."></textarea>
                        </div>
                    </div>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="closedeliverymark()" data-dismiss="modal">Close</button>
                    <button id="savedeliverytype" type="button" class="btn btn-primary">Save changes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $('#view_more').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "Admin_Delivery.aspx/getCustomerOrdersbydatesNextRecords",
        ...
        //        type: "POST",
        //        async: true,
        //        url: "Admin_Delivery.aspx/getCustomerOrdersbydates",
        ...
        function All_Actions() {
            ...
            $(".trackorderbtn").click(function (e) {
                ...
                var img = "";
                if (url != "") {
                     img = '<img src="/DispatchOrderPicture/' + url + '" style="width:100%" />';
                }
                var a = img + '<p>' + cmnt + '</p>';
                $("#ImagePreview").empty();
            ...
            $(".show_subscribed_items").click(function () {
                ...
                if ($.trim(current.parent().parent().parent().find(".items__append").html()).length == 0) {
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "Admin_Delivery.aspx/getItemsBySchedule",
                ...
                function payamountfunction(recievedamt, MOP, Header_ID) {
                    if (recievedamt == '0') {
                        ...
                        $.ajax({
                            type: "POST",
                            async: true,
                            url: "Admin_Delivery.aspx/AmountReceived",
            ...
            function getDataByHeaderID(Header_ID, current) {
                ...
                $.ajax({
                    type: "POST",
                    async: true,
                    url: "Admin_Delivery.aspx/getCustomerOrdersbydatesRefresh",
            ...
            $('.assign_to_select').click(function (e) {
                ...
                if (current.find('option').length == 1) {
                    ...
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "Admin_Delivery.aspx/getDeliveryBoysList",
                        data: '{Name:"' + Name + '"}',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        beforeSend: function () {
            ...
            $('.assign_to_select').change(function (f) {
                ...
                $.ajax({
                    type: "POST",
                    async: true,
                    url: "Admin_Delivery.aspx/AssignDeliveryBoy",
            ...
            $('.notifybtnshow').click(function (e) {
                ...
                $(".notifycustbtn").click(function (e) {
                    ...
                    $.ajax({
                        type: "POST",
                        async: true,
                        url: "Admin_Delivery.aspx/NotifyCustomer",
                        ...
                        buttons: {
                            ...
                            Yes: function () {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "Admin_Delivery.aspx/InitiateRefund",
            ...
            $('.delievrymark').click(function (e) {
                e.stopImmediatePropagation();
                e.preventDefault();
                var headerid = $(this).attr('headerid');
                var ordernumber = $(this).attr('order_no');
                $(this).text('');
                $(this).append('<i class="fa fa-check"></i> Out for Delivery');
                $("#DeliveryModal").modal();
                $("#savedeliverytype").attr('headerid', headerid);
                $("#savedeliverytype").attr('ordernumber', ordernumber);
        ...
        $(".Deliverytype").click(function (e) {
            e.stopImmediatePropagation();
            e.preventDefault();
            $(".Deliverytype").removeClass('deliveryactive');
            $(this).toggleClass("deliveryactive");
        });


        function closedeliverymark() {
            $(".Deliverytype").removeClass('deliveryactive');
        }
        ...
        $("#DispatchPicture").change(function () {
            readURL(this);
        });

        $("#savedeliverytype").click(function (e) {
            e.stopImmediatePropagation();
            e.preventDefault();
            var hedid = $(this).attr('headerid');
            var odrid = $(this).attr('ordernumber');
            var Comment = $.trim($("#DispatchComments").val());
            var PictureUrl = $("#imagePreview").attr('src');
            var ImgSource = '';
            if (PictureUrl != null || PictureUrl != undefined) {
                ImgSource = PictureUrl.replace(/^data:image\/(png|jpg|jpeg|gif);base64,/, "");
            }

            var type = "";
            if ($(".Deliverytype").hasClass('deliveryactive')) {
                type = $(".deliveryactive").attr('typeofdel');

            }
            if (type == "") {
                showpopup('please select dispactch type.');
            } else {
                fnorderdeliveredmark(hedid, '', 'Y', odrid, type, Comment, ImgSource);
            }

        });
        function fnorderdeliveredmark(Header_ID, Line_ID, Flag, Order_No, deliverytype, Comment, ImgSource) {
            ...
            $.ajax({
                type: "POST",
                async: true,
                url: "Admin_Delivery.aspx/MarkDeliveredforbtn",
                data: Data,
                //data: '{Header_ID:"' + Header_ID + '",Line_ID:"' + Line_ID
                //    + '",Flag:"' + Flag + '",Order_No:"' + Order_No + '",deliverytype:"' + deliverytype+'"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    showpopup('Order marked as "Dispactched" successfully');
                    $('#DeliveryModal').modal('hide');
                    var Date = $('#reportrange span').text();
                    Date = Date.split(' - ');
                    getData($.trim(Date[0]), $.trim(Date[0]));
                }
            });
        }
        ...
        function fnParticularItemCheckedUnchecked(Header_ID, Line_ID, Flag, Order_No) {
            $.ajax({
                type: "POST",
                async: true,
                url: "Admin_Delivery.aspx/MarkDelivered",
        ...
        function todaysCollection() {
            $.ajax({
                type: "POST",
                async: true,
                url: "Admin_Delivery.aspx/todaysCollection",
                data: '{}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
        ...
        function Invoice(Header_ID, Invoice, Flag) {
            $.ajax({
                type: "POST",
                async: true,
                url: "Admin_Delivery.aspx/Invoice",
        ...
        $("#search").autocomplete({
            source: function (request, response) {
                var Flag = $('#SelectBy option:selected').val();
                if (Flag == 'header') {
                    return;
                }
                $.ajax({
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    url: "PendingDelivery.aspx/getSuggestionMobileNo",
                    dataType: "json",
                    ...
                    success: function (data) {
                        if (data.d != '') {
                            response($.map($.parseJSON(data.d), function (item) {
                                ...
                                AC.value = item.C_NUMBER;
                                AC.label = item.C_NUMBER + ' - ' + item.C_Name;
                                return AC
                            }));
                        }
        ...
        function getDataByClick() {
            var Mobile = $.trim($('#search').val());
            var Flag = $('#SelectBy option:selected').val();
            if (Mobile.length == 0) {
                showpopup('Please enter mobile number or order no');
                return;
            }
            if (Flag == 'mob' && Mobile.length != 10) {
                showpopup('Please enter 10 digits mobile number');
                return;
            }
            $.ajax({
                type: "POST",
                async: true,
                url: "PendingDelivery.aspx/getDataByMobileNo",
                data: '{Mobile:"' + Mobile + '",Flag:"' + Flag + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                beforeSend: function () {
                    $("#preload").show();
