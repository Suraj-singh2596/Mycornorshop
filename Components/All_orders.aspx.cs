...
public partial class Components_All_orders : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            string QstringA = Request.QueryString.ToString();
            string QueryStringA = Cl_admin.Decrypt(HttpUtility.UrlDecode(QstringA));
            string RID = HttpUtility.ParseQueryString(QueryStringA).Get("rid").Trim();
            string UserId = HttpUtility.ParseQueryString(QueryStringA).Get("userid").Trim();
    ...
    [WebMethod]
    public static string getParticularOrder(string Url)
    {
        string QueryString = Cl_admin.Decrypt(Url);
        string HeaderId = HttpUtility.ParseQueryString(QueryString).Get("headerid").Trim();
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow DR in ds.Tables[0].Rows)
            {
                ...
                Orders = Orders + "<div class=\"card container p-1 mt-2\"><div class=\"row\" header_id='" + DR["HEADER_ID"].ToString() + "'><div class=\"col-md-12\"><div class=\"container\">" +
                                 ...
                                 "</div></div><div class=\"col-7 col-md-7 \"><div class=\"paymentbox\"><span>" + status + "</span></div>" +
                                 "</div>" +
    ...
    public static string getCustomerOrderByDate(string startdate, string enddate)
    {
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow DR in ds.Tables[0].Rows)
            {
                ...
                if (DR["TrackId"].ToString() != "")
                {
                    trackbtn = "<div class=\"col-6\"><button trackid=\"" + DR["TrackId"].ToString() + "\" comnt=\"" + DR["DispatchComments"].ToString() + "\" imgurl=\"" + DR["PictureUrl"].ToString() + "\" class=\"trackorderbtn w-100 mb-2 mt-2\">Track Order</button></div>";
                }
