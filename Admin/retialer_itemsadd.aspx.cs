...
public partial class Admin_retialer_itemsadd : System.Web.UI.Page
{
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            if (Flag == "Y")
            {
                tr = "<tr role=\"row\" class=\"odd\">" +
                   ...
                   "<td>" + Price + "</td><td>" + ds.Tables[0].Rows[0]["SUB_CATEGORY"].ToString() + "</td><td><button type=\"button\" class=\"btn btnstyle Remove\" mid='" + ds.Tables[0].Rows[0]["MID"].ToString() + "'>Remove</button></td></tr>";
            }
            if (Flag == "N")
            {
                tr = "<tr role=\"row\" class=\"odd\">" +
